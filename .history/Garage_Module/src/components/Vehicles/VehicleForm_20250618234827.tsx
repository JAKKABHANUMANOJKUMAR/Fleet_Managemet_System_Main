...
export default function VehicleForm({ vehicle, onClose, onSave, isDarkMode = false }: VehicleFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Validate required tyre numbers
    const requiredTyres = ['tyre1', 'tyre2', 'tyre3', 'tyre4'];
    const missingTyres = requiredTyres.filter(tyre => !formData.tyres[tyre as keyof typeof formData.tyres].number.trim());
    
    if (missingTyres.length > 0) {
      alert(`Please fill in the tyre numbers for: ${missingTyres.map(t => t.replace('tyre', 'Tyre ')).join(', ')}`);
      return;
    }
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            <p className={`text-sm mb-4 ${
              ...
              <span className="text-red-500">*</span> Tyres 1, 2, 3, and 4 are mandatory. Tyres 5 and 6 are optional.
            </p>
