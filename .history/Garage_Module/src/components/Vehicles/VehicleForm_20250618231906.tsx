...
export default function VehicleForm({ vehicle, onClose, onSave, isDarkMode = false }: VehicleFormProps) {
  const [formData, setFormData] = useState({
    ...
    tyres: {
      tyre1: {
        ...
        photo: vehicle?.tyres?.tyre1?.photo || null
      },
      tyre2: {
        number: vehicle?.tyres?.tyre2?.number || '',
        photo: vehicle?.tyres?.tyre2?.photo || null
      },
      tyre3: {
        number: vehicle?.tyres?.tyre3?.number || '',
        photo: vehicle?.tyres?.tyre3?.photo || null
      },
      tyre4: {
        number: vehicle?.tyres?.tyre4?.number || '',
        photo: vehicle?.tyres?.tyre4?.photo || null
      },
      tyre5: {
        number: vehicle?.tyres?.tyre5?.number || '',
        photo: vehicle?.tyres?.tyre5?.photo || null
      },
      tyre6: {
        number: vehicle?.tyres?.tyre6?.number || '',
        photo: vehicle?.tyres?.tyre6?.photo || null
      }
    }
  });
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (name.includes('tyres.')) {
      const [tyres, tyreKey, field] = name.split('.');
      setFormData(prev => ({
        ...prev,
        tyres: {
          ...prev.tyres,
