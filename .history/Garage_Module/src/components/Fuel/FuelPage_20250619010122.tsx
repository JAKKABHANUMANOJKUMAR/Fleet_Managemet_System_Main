            ...
            <form onSubmit={(e) => { e.preventDefault(); handleAddFuel(); }} className="p-6 space-y-6">
              ...
              <div className="max-w-3xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  {/* Tyres Numbers 1-6 */}
                  {[1,2,3,4,5,6].map(num => (
                    <div key={`tyresNumber${num}`}>
                      <label className={`block text-sm font-medium mb-2 ${
                        isDarkMode ? 'text-gray-300' : 'text-gray-700'
                      }`}>{`Tyres number ${num}`} <span className="text-red-500">*</span></label>
                      <input
                        type="text"
                        value={String(newEntry[`tyresNumber${num}` as keyof FuelEntryWithTyres] || '')}
                        onChange={e => setNewEntry({ ...newEntry, [`tyresNumber${num}`]: e.target.value })}
                        required
                        className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors duration-200 ${
                          isDarkMode 
                            ? 'bg-gray-700 border-gray-600 text-white placeholder-gray-400' 
