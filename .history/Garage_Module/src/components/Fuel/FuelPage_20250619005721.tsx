                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  {[1,2,3,4,5,6].map(num => (
                    <div key={`tyre${num}`}>
                      <label className={`block text-sm font-medium mb-2 ${
                        isDarkMode ? 'text-gray-300' : 'text-gray-700'
                      }`}>{`Tyre Number ${num} *`}</label>
                      <input
                        type="text"
                        value={newEntry[`tyreNumber${num}` as keyof FuelEntryWithTyres] || ''}
                        onChange={e => setNewEntry({ ...newEntry, [`tyreNumber${num}` as keyof FuelEntryWithTyres]: e.target.value })}
                        required
                        className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors duration-200 ${
                          isDarkMode 
                            ? 'bg-gray-700 border-gray-600 text-white placeholder-gray-400' 
