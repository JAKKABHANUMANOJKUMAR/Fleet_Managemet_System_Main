...
import { mockFuelEntries, mockBranches, mockVehicles } from '../../data/mockData';
...
type FuelEntryWithTyres = Partial<FuelEntry> & {
  tyresNumber1?: string;
  tyresNumber2?: string;
  tyresNumber3?: string;
  tyresNumber4?: string;
...
function getTyreNumber(vehicle: typeof mockVehicles[number] | undefined, num: number): string {
  if (!vehicle || !vehicle.tyres) return '';
  switch (num) {
    case 1: return vehicle.tyres.tyre1?.number || '';
    case 2: return vehicle.tyres.tyre2?.number || '';
    case 3: return vehicle.tyres.tyre3?.number || '';
    case 4: return vehicle.tyres.tyre4?.number || '';
    case 5: return vehicle.tyres.tyre5?.number || '';
    case 6: return vehicle.tyres.tyre6?.number || '';
    default: return '';
  }
}
...
export default function FuelPage({ isDarkMode = false }: FuelPageProps) {
  ...
  const [newEntry, setNewEntry] = useState<FuelEntryWithTyres>({});
  const [selectedBranch, setSelectedBranch] = useState('all');
  const [tyreVerification, setTyreVerification] = useState<{ [key: string]: 'unverified' | 'verified' | 'issue' }>({
    tyresNumber1: 'unverified',
    tyresNumber2: 'unverified',
    tyresNumber3: 'unverified',
    tyresNumber4: 'unverified',
    tyresNumber5: 'unverified',
    tyresNumber6: 'unverified',
  });
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className={`rounded-xl shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto m-4 transition-colors duration-300 ${
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleAddFuel(); }} className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      onChange={e => {
                        const selectedVehicle = mockVehicles.find(v => v.registrationNumber === e.target.value);
                        setNewEntry({
                          ...newEntry,
                          vehicleNumber: e.target.value,
                          vehicleId: selectedVehicle ? selectedVehicle.id : '',
                          driverId: selectedVehicle?.driverId || '',
                          branch: selectedVehicle && selectedVehicle.branchId ? (mockBranches.find(b => b.id === selectedVehicle.branchId)?.name || '') : '',
                          tyresNumber1: selectedVehicle?.tyres?.tyre1?.number || '',
                          tyresNumber2: selectedVehicle?.tyres?.tyre2?.number || '',
                          tyresNumber3: selectedVehicle?.tyres?.tyre3?.number || '',
                          tyresNumber4: selectedVehicle?.tyres?.tyre4?.number || '',
                          tyresNumber5: selectedVehicle?.tyres?.tyre5?.number || '',
                          tyresNumber6: selectedVehicle?.tyres?.tyre6?.number || ''
                        });
                      }}
                      required
                  ...
                  {/* Tyres Numbers 1-6 */}
                  {[1,2,3,4,5,6].map(num => {
                    const key = `tyresNumber${num}`;
                    const selectedVehicle = mockVehicles.find(v => v.registrationNumber === newEntry.vehicleNumber);
                    const autoValue = getTyreNumber(selectedVehicle, num);
                    return (
                      <div key={key} className="flex items-center gap-2">
                        <div className="flex-1">
                          <label className={`block text-sm font-medium mb-2 ${
                            isDarkMode ? 'text-gray-300' : 'text-gray-700'
                          }`}>{`Tyres number ${num}`} <span className="text-red-500">*</span></label>
                          <input
                            type="text"
                            value={String(newEntry[key as keyof FuelEntryWithTyres] || '')}
                            onChange={e => {
                              setNewEntry({ ...newEntry, [key]: e.target.value });
                              setTyreVerification(v => ({ ...v, [key]: 'unverified' }));
                            }}
                            required
                            className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors duration-200 ${
                              isDarkMode 
                        ...
                        {tyreVerification[key] === 'unverified' && (
                          <>
                            <button
                              type="button"
                              className={`ml-2 px-3 py-1 rounded-lg text-xs font-semibold border transition-colors duration-200 ${
                                isDarkMode
                                  ? 'bg-gray-700 border-gray-500 text-gray-300 hover:bg-blue-700 hover:text-white'
                                  : 'bg-gray-100 border-gray-300 text-gray-700 hover:bg-blue-600 hover:text-white'
                              }`}
                              onClick={() => {
                                const currentValue = String(newEntry[key as keyof FuelEntryWithTyres] || '');
                                if (currentValue === autoValue) {
                                  setTyreVerification(v => ({ ...v, [key]: 'verified' }));
                                } else {
                                  setTyreVerification(v => ({ ...v, [key]: 'issue' }));
                                  setTimeout(() => {
                                    const issues = [1,2,3,4,5,6].filter(n => {
                                      if (n === num) return true;
                                      return tyreVerification[`tyresNumber${n}`] === 'issue';
                                    });
                                    setRemarks(`Tyres Number is Changed (${issues.map(n => `Tyre number ${n}`).join(', ')})`);
                                  }, 0);
                                }
                              }}
                            >
                            ...
                            <button
                              type="button"
                              className={`ml-2 px-3 py-1 rounded-lg text-xs font-semibold border border-red-400 text-red-700 bg-red-50 hover:bg-red-100 transition-colors duration-200`}
                              onClick={() => {
                                setTyreVerification(v => ({ ...v, [key]: 'issue' }));
                                setTimeout(() => {
                                  const issues = [1,2,3,4,5,6].filter(n => {
                        ...
                        {tyreVerification[key] === 'verified' && (
                          <span className="ml-2 text-green-600 text-xs font-semibold">Verified</span>
                        )}
                        {tyreVerification[key] === 'issue' && (
                          <span className="ml-2 text-red-600 text-xs font-semibold">Issue</span>
                        )}
                      </div>
                    );
                  })}

                  {/* Remarks field if any issue */}
                  {Object.values(tyreVerification).includes('issue') && (
                    <div className="col-span-2">
                      ...
                      <textarea
                        ...
                        rows={2}
                        placeholder="Please specify the issue(s) with the tyre number(s)"
                      />
                    </div>
                  )}
                </div>
              ...
              {/* Form Actions */}
              <div className={`flex items-center justify-center space-x-4 pt-6 border-t transition-colors duration-300 ${
                isDarkMode ? 'border-gray-700' : 'border-gray-200'
              }`}>
                <button
                  type="button"
